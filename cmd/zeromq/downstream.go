...
var DownstreamCommand = &cobra.Command{
	Use:   `downstream`,
	Short: `Consume events from 0MQ broker`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		go func() {
			...
			for setup.Ctx.Err() == nil {
				...
				if err == nil {
					tmp := msg.Data()
					tmp[len(tmp)-1] = []byte(fmt.Sprintf("downstream-%s: %d", id, i))
					err = client.Push(dto.NewByteMsg(tmp...))
					for err != nil && setup.Ctx.Err() == nil {
						err = client.Push(dto.NewByteMsg(tmp...))
					}
				}
